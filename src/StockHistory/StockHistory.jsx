  ...
  useEffect(() => {
      async function getAnalysisHistory() {
        for (const stock of stocks) {
          ...
          await fetch(`${RETRIEVAL_ENDPOINT}/v2/retrieve/${username}/finance/${stockName}`)
          .then((res) => res.json())
          .then((data) => {
  ...
  if (!gotStockAnalysis) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center px-4 py-10">
        <div className="border-b-4 border-indigo-500  text-white text-5xl font-semibold mt-[2%] mb-[3%]">Previous Stocks</div>
        <div className="text-white mb-[1%] text-[24px]">Getting your stock history now</div>
        <div role="status">
    ...
    return (
      <>      
        ...
        <div className="min-h-screen bg-black flex flex-col items-center justify-center px-4 py-10">
        <div className="border-b-4 border-indigo-500  text-white text-5xl font-semibold mt-[2%] mb-[3%]">Previous Stocks</div>
        {stockAnalysis.map((stock, i) => {
          const values = []
          for (const event of stock) {
            values.push({date: event.time_object["time-stamp"], price: event.attribute.close});
          }
          console.log(values)
          ...
          return (
            ...
            <h1 key={`h1`} className="text-3xl font-bold text-center mb-6 text-neutral-800 text-white">
              {stock[0].attribute["stock_name"].toUpperCase()} Stock Data
            </h1>
            <div key={`div`} className="w-full max-w-4xl border-4 border-orange-400 bg-white dark:bg-neutral-800 rounded-lg shadow p-6 mb-10 hover:shadow-xl hover:shadow-orange-400/50 ">
            <Line
              key={`Line`}
              data={chartData}
              options={{
