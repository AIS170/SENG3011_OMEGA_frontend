...
export default function StockInput() {
  ...
  async function analyseStockName(e) {
    ...
    try {
      ...
      await fetch(`${COLLECTION_ENDPOINT}/news?name=${name.toLowerCase()}`);
      ...
      const events = stockDataRetrieval?.events || [];
      const labels = events.map((entry) => entry.time_object["time-stamp"]);
      const prices = events.map((entry) =>
        parseFloat(entry.attribute?.close || 0)
      );
      ...
      //       ],
      //     },
      //     analysis: analysis?.result || "No analysis available.",
    ...
    <>
      ...
      <div className="min-h-screen bg-black">
        <div className="flex items-center justify-center flex-wrap">
          <div className="text-center w-full font-semibold text-[50px] mt-10 text-white">
          ...
          <div className="p-8">
            ...
            >
              <input
                ...
                className="bg-white rounded-sm p-2 border-2 border-gray-400 unselectable focus:border-orange-400"
                type="text"
                required
              />
              <br />
              <button
                type="submit"
                className="mt-4 hover:cursor-pointer rounded bg-gradient-to-r from-orange-400 via-pink-500 to-purple-600 px-6 py-2 text-white shadow-lg transition hover:shadow-xl submitButton"
              >
                Analyse
              </button>
              ...
              {loadingResults && (
                <>
                  <div className="text-white mt-6">One moment please...</div>
        ...
        {error && (
          <div className="text-center text-red-600 bg-white p-4">
